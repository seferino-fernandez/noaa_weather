...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📍 Getting weather information for coordinates: {}", coordinates);

    // 1. Get point metadata
    println!("\n1️⃣ Getting point metadata...");
    match points::get_point(&config, coordinates).await {
        Ok(point_data) => {
            if let Some(properties) = &point_data.properties {
    ...
    match alerts::get_active_alerts(&config, None, None, None, Some(5), None, None, None).await {
        Ok(alerts_data) => {
    ...
    match points::get_point_stations(&config, coordinates).await {
        Ok(stations_data) => {
            ...
            if let Some(first_station) = stations_data.features.first() {
                if let Some(station_id) = &first_station.properties.station_identifier {
                    println!("  📡 Getting latest observation from station: {}", station_id);

                    match stations::get_latest_observation(&config, station_id, None).await {
                        Ok(observation) => {
                            if let Some(props) = &observation.properties {
                                println!("    ✅ Station: {}",
                                    props.station_name.as_deref().unwrap_or("Unknown")
                                );

                                if let Some(temp) = &props.temperature {
                                    if let Some(temp_value) = temp.value {
    ...
    println!("   cargo run --example weather_alerts");
    println!("   cargo run --example aviation_weather");
